...
namespace SplittableDataGridSAmple.Tabs
{
    public sealed partial class InventorPrintTab : TabViewItem, Interfaces.IInitTab, INotifyPropertyChanged
    {
                    ...
                    foreach (var file in items)
                    {
                        ...
                        if (!Directory.Exists(file.Path)) //si ce n'est pas un répertoire
                        {
                            if (file.Name.EndsWith(".idw"))
                            {
                                ...
                                IDWPrintModels.Add(new IDWPrintModel(fileInfo, nbPDFDXFPropertyChanged));
                            }
                        }
        ...
        {
            var IDWModelContext = ((FrameworkElement)sender).DataContext as IDWPrintModel;
            if (IDWModelContext != null)
            {
                ...
                if (bitmapImage != null)
                {
                    ...
                    ThumbNailDescription.Text = sheetSizeConverter.Convert(IDWModelContext.SheetSize,typeof(string),null,"").ToString();
                    ThumbNailCustomer.Text = sheetOrientationConverter.Convert(IDWModelContext.Orientation,typeof(string),null,"").ToString();
